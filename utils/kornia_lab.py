# The code is taken from here: https://github.com/kornia/kornia/issues/239
...
class RgbToLab(nn.Module):
    ...
    def rgb_to_lab(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if not(len(input.shape) == 3 or len(input.shape) == 4):
            raise ValueError("Input size must have a shape of (*, 3, H, W) or (3, H, W). Got {}".format(input.shape))

        if input.shape[-3] != 3:
            raise ValueError("Expected input to have 3 channels, got {}".format(input.shape[-3]))
